...
import { X, RefreshCw, Check, MessageSquare, Mail, User } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export const UserProfileModal = ({ isOpen, onClose, targetUser, onStartDm }) => {
  const { user, setUser } = useAuth();
  
  const isSelf = !targetUser || targetUser.id === user?.id;
  const activeUser = isSelf ? user : targetUser;

  const [displayName, setDisplayName] = useState(activeUser?.display_name || '');
  const [statusText, setStatusText] = useState(activeUser?.status || '');
  const [bioText, setBioText] = useState(activeUser?.bio || 'Software Engineer & Team Collaborator');
  const [avatarUrl, setAvatarUrl] = useState(activeUser?.avatar_url || '');
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        ...
        >
          {/* Header Banner */}
          <div className="relative h-28 bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-600 flex items-end justify-between p-4">
            <button
              onClick={onClose}
          ...
          <div className="px-6 pb-6 relative">
            <div className="flex justify-between items-end -mt-12 mb-4">
              <div className="relative">
                <img
                  src={avatarUrl || activeUser.avatar_url}
                  alt="Avatar"
            ...
            {isSelf ? (
              ...
              <div className="space-y-4 pt-2">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">{activeUser.display_name}</h3>
                  <p className="text-xs text-indigo-500 font-semibold">{activeUser.status || "Online"}</p>
                </div>

                <div className="p-4 rounded-2xl bg-gray-50 dark:bg-slate-800/80 border border-gray-200 dark:border-slate-800">
                  <span className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">
                    About / Bio
                  </span>
                  <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">
                    {activeUser.bio || "Software Engineer & Team Collaborator"}
                  </p>
                </div>
              </div>
            )}
